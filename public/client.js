// client.js (Three.js version - Multi-room ready) - updated for spectator queue & postgame flow
...
const resultContinueBtn = document.getElementById('resultContinueBtn'); // 続ける
const resultQuitBtn = document.getElementById('resultQuitBtn'); // やめる
...
createRoomBtn.addEventListener("click", () => {
  ...
  const joinData = { room: roomVal, name: nameVal };
...
socket.on("cheer", (msg) => {
  // handle cheer separately but also show as flying comment
  appendChat(msg);
  launchFlyingComment(msg.text);
});
...
function render(stateObj) {
  ...
  // watchQueue UI
  renderWatchQueue(state.watchQueue || []);
...
function renderWatchQueue(queue) {
  if (!watchListEl) return;
  watchListEl.innerHTML = '';
  if (!queue || !queue.length) {
    watchListEl.textContent = '観戦者: なし';
    return;
  }
...
function highlightSelection(meshToHighlight = null) {
  ...
  if (selectedMesh) {
    try { selectedMesh.material.color.set(COLORS[selectedMesh.userData.owner]); } catch(e){}
    selectedMesh = null;
  }
  if (meshToHighlight) {
    try { meshToHighlight.material.color.set(COLORS.selected); } catch(e){}
...
// --- リザルト画面の処理（結果表示 + 続行/やめる） ---
function showResult(winner) {
  resultOverlay.classList.remove('hidden');
  resultContent.classList.remove('lose'); // クラスリセット

  // reset buttons
  resultContinueBtn.disabled = false;
  resultQuitBtn.disabled = false;
  resultContinueBtn.textContent = '対戦続ける';
  resultQuitBtn.textContent = '対戦やめる';
...
if (resultContinueBtn) {
  resultContinueBtn.addEventListener('click', () => {
    // send continue decision
    socket.emit('postgame_decision', { action: 'continue' }, (ack) => {
      if (ack && ack.ok) {
        addLog('「続ける」を選択しました。相手の選択を待ちます。');
        resultContinueBtn.disabled = true;
        resultContinueBtn.textContent = '選択済み（続ける）';
      } else if (ack && ack.error) {
        addLog('選択失敗: ' + ack.error);
      }
    });
  });
}
if (resultQuitBtn) {
  resultQuitBtn.addEventListener('click', () => {
    socket.emit('postgame_decision', { action: 'quit' }, (ack) => {
      if (ack && ack.ok) {
        addLog('「やめる」を選択しました。処理中...');
        resultQuitBtn.disabled = true;
        resultQuitBtn.textContent = '選択済み（やめる）';
      } else if (ack && ack.error) {
        addLog('選択失敗: ' + ack.error);
      }
    });
...
socket.on('assign', (d) => {
  if(d && d.slot) {
    ...
    // If our role changed to player, update mySlot
    if (socket.id === myId) {
      mySlot = d.slot;
...
socket.on('postgame_update', (d) => {
  addLog(`(System) ${d.slot} が ${d.action} を選択しました`);
  // optionally show waiting UI; update_state will be emitted by server as well
});

socket.on('force_leave_after_quit', () => {
  // Server asked us to leave (we selected quit)
  addLog('サーバーの指示により退室します');
  // In practice, simply reload to home
  window.location.href = window.location.pathname;
});
